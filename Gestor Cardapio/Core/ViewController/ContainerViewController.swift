...
extension ContainerViewController: SideMenuDelegate {
	...
	func leftButtonTapped() {
		if let controller = rootViewController as? StartViewController {
			controller.navigationDelegate?.leftButtonTap()			
		}
    ...
    func itemSelected(item: ContentViewControllerPresentation, sideMenuItem: SideMenuItem) {
        if sideMenuItem.name == "Venda conosco sem taxas" {
            if let url = URL(string: "https://www.cardapio.ai") {
